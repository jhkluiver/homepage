...
const webmail = process.env.webmail; // || 'localhost:3080';
const mailmonitor = process.env.mailmonitor; // || 'localhost:4200';
const mailgateway = process.env.mailgateway; // || 'localhost:4200';
...
console.log(`Homepage (https://github.com/STRATEGY-EU/homepage) version 21 feb 2022 10:30.`)
...
if (webmail) {
  console.log(`Map external '${hostname}/mail/*' --> to internal '${webmail}/*'`);
  proxy.register(`${hostname}/webmail`, webmail);
} else console.log(`No proxy for webmail service configured.`);

if (mailmonitor) {
  console.log(`Map external '${hostname}/mailmonitor/*' --> to internal '${mailmonitor}/*'`);
  proxy.register(`${hostname}/mailmonitor`, mailmonitor);
} else console.log(`No proxy for mail monitor service configured.`);

if (mailgateway) {
  console.log(`Map external '${hostname}/mailgateway/*' --> to internal '${mailgateway}/*'`);
  proxy.register(`${hostname}/mailgateway`, mailgateway);
} else console.log(`No proxy for mail gateway service configured.`);
